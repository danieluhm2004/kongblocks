		...
		if (label.equalsIgnoreCase("보호")) {
			if (sender instanceof Player) {
				...
				if (args.length == 0) {
					...
					MessageSender.playerMessage(player,
							KongBlocks.getPrefix() + "&9/보호 &6리뷰 <메세지> &f- 해당 플러그인을 리뷰합니다.");
					MessageSender.playerMessage(player,
							KongBlocks.getPrefix() + "&9/보호 &6저장 &f- 블럭 데이터를 저장합니다. &c&l(OP)");
					...
					if (KongBlocks.informationEnable) {
						if (!KongBlocks.information.contains(player)) {
							...
							MessageSender.playerMessage(player,
									KongBlocks.getPrefix() + KongBlocks.informationEnableMessage);
						} else {
							KongBlocks.information.remove(player);
							MessageSender.playerMessage(player,
									KongBlocks.getPrefix() + KongBlocks.informationDisableMessage);
						}
					} else {
						MessageSender.playerMessage(player,
								KongBlocks.getPrefix() + KongBlocks.informationNotSupportMessage);
					}
					...
					if (player.isOp()) {
						try {
							...
							MessageSender.playerMessage(player, KongBlocks.getPrefix()
									+ "다음 &c오류&f로 인하여 &e파일&f을 &6저장&f할 수 없습니다. " + e.getMessage());
						}
					} else {
